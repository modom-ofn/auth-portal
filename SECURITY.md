...
- **Runtime hygiene:** containers are built with Docker Hardened Images for both build and runtime (`dhi.io/golang:1.26.4-alpine3.23-dev` -> `dhi.io/alpine-base:3.23-alpine3.23`), keep only CA certs/tzdata in the final layer, and run as non-root UID 65532 to shrink the attack surface.
